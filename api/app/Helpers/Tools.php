...
class Tools
{
    ...
    public function b64toFile($folderName, $b64file, $filename = 'myfile', $ext = null)
    {
        ...
        if (count($type) <= 1) {
            throw new TransactionException('Invalid b64 string');
        }
    ...
    /*  Este metodo busca que todos los elementos en
    el primer parametro sean requeridos por la lista del segundo.
    si faltan campos regresa un response con un codigo de error,
    si no falta ninguno regresara null
    asignado en el tercer parametro
     */

    public function checkRequired($requestData, $required, $httpCode)
    {
        ...
        return (count($errors) > 0) ? response($errors, $httpCode) : true;
    }
    /**
     * funcion que salva un archivo y regresa su URL
     *
     * @param [type] $photo
     * @param [type] $path
     * @return string
     */
    public function saveFile($photo, $path)
    {
        $path = public_path() . $path;

        $filePathData = preg_split("/\//", $path);
        $fileName = end($filePathData);
        $folderPath = str_replace($fileName, "", $path);
        $folderPath = str_replace("//", "/", $folderPath);
        $fileName = preg_split("/\./", end($filePathData));
        $name = $fileName[0];
        $ext = end($fileName);
        ...
        return str_replace(
            ...
            '',
            $this->b64toFile($folderPath, $photo, $name, $ext)
        );
    }
    /**
